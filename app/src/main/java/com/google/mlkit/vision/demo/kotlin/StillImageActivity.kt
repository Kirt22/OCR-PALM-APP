...
import com.google.mlkit.vision.demo.api.BARDinterface
...
import kotlinx.android.synthetic.main.activity_still_image.promptOutput
...
@KeepName
class StillImageActivity : AppCompatActivity() , TextRecognitionProcessor.TextForBardCallBack{
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    Log.i("StillImageActivity", "onCreate: Entered graphicOverlay")
  ...
  override fun sendToActivity(textFromImage: String) {
    sendToBard(textFromImage)
  }

  private fun sendToBard(inputPrompt: String) {

    val APIinterface = RetrofitClient.getInstance().create(BARDinterface::class.java)

    APIinterface.getResponse(inputPrompt).enqueue(object: retrofit2.Callback<JsonObject> {

      override fun onResponse(call: Call<JsonObject>, response: retrofit2.Response<JsonObject>) {
        val promptOutput = findViewById<TextView>(R.id.promptOutput)
        promptOutput.text = response.body().toString()
      }

      override fun onFailure(call: Call<JsonObject>, t: Throwable) {
        Toast.makeText(this@StillImageActivity, "API Failed", Toast.LENGTH_SHORT).show()
      }
